...
Connection.prototype.release = function(suppressReset) {
    this.pool.release(this, suppressReset);
};
...
ConnectionPool.prototype.release = function(connection, suppressReset) {
    if (this.drained) //pool has been drained
        return;
    ...
    for (i = self.connections.length - 1; i >= 0; i--) {
        ...
        if (pooled.con === connection) {
            ...
            var callbackForReset = (function (err) {
            ...
            if(suppressReset) {
                //console.log('avoiding reset!!!!!!!!');
                process.nextTick(callbackForReset);
            } else {
                connection.reset(callbackForReset);
            }
